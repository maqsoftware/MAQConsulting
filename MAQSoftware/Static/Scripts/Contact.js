(function () {
    ...
    function ShowTurns(route) {
        ...
        if (route) {            
            ...
            var turns = "<ol class='HYDDirection'>";
            var leg = null;
            var i;
            var j;
    ...
    function getRouteTo(latitude, longitude, inputAddress) {
        var MAQSoftwareLocation = new VELatLong(latitude, longitude);
        if (inputAddress && inputAddress !== "") {
            getRouteMap([inputAddress, MAQSoftwareLocation]);
        }
        var iAddressSectionTopPosition = $("#NHYDTtl").offset().top;
    ...
    function ResizeMap() {
        var oMap = $("#myMap");
        oMap.css("width", "100%");
        var iMapWidth = oMap.width();
        var iMapHeight = (iMapWidth * iHeight) / iWidth;
